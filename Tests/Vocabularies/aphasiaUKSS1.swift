...
final class e2e_TCHDWPLaunchaphasiaUKSS: XCTestCase {
    ...
    func testLaunchaphasiaUKSS() throws {
        ...
        XCUIApplication().tables.staticTexts["Aphasia UK SS  "].tap()
        
        let elements = app.buttons.allElementsBoundByIndex
        let eighthElement = elements[7]
        let aphasiaUKWord = app.buttons["Places"]
        
        XCTAssertGreaterThan(elements.count, 7, "There are not enough elements")
        XCTAssertEqual(eighthElement.label, "Places", "The title is correct")
        XCTAssertTrue(aphasiaUKWord.exists);
        ...
        let sdbElement = app.textFields["Supermarket "]
